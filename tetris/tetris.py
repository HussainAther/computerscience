from graphics import CanvasFrame, Point, Rectangle, Text, Window
import random

DELAY = 500 # ms before block moves down one space

class Block(Rectangle):
    """
    Block class:
    Implement a block for a tetris piece
    Attributes: x - type: int, y - type: int
...
class Board():
    ...
    height - type:int - height of the board in squares, canvas - type:CanvasFrame -
    where the pieces will be drawn, grid - type:Dictionary - keeps track of the
    current state of the board; stores the blocks for a given position
    """
    ...
    def can_move(self, x, y):
        ...
        if the position is outside of the board boundaries, can't move there, return False
        2. if there is already a block at that postion, can't move there, return False
